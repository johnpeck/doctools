...
\usepackage{subcaption}      % For adding figures within figures
...
\usepackage{supertabular}   % For continuing tables in a second column
...
\usepackage{siunitx}
% sisetup takes a comma-separated list of key = value pairs
\sisetup{%
  alsoload = synchem
}

% Allows using greek letters outside of math mode
...
% Provides \lstlisting for code formatting
\usepackage{listings}
...
% normal page style
%
% The \normallefthead and \normalrighthead commands need to be renewed
% when you want them to change -- inside the document.  Latex doesn't
% seem to have a good way to extract the section title.
